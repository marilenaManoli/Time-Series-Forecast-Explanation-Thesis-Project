...
1. **Look only at the Metrics & quality tab.** "Without reading anything else, which model would you trust for next week's operational planning, and why?" *(captures their judgment from raw numbers + badges, before seeing any generated text. Note: the MAE and RMSE cells now also show a skill score vs. Seasonal Naive baseline — e.g. "+19% vs. SNaive" — as a small coloured sub-line. Do not draw the participant's attention to it; observe whether they notice it and how they interpret it.)*
2. **Now open the Linguistic explanations tab for that same model, then for the model ranked worst.** "Does this sentence tell you anything the table didn't? Does anything here seem inaccurate or overstated?" *(tests accuracy/sufficiency of the structured-explanation layer — domain experts especially should be pushed here: "would you ever disagree with this sentence given the numbers?" Note: below the CwW-generated sentence, each model now also has a hand-authored "Model profile" block with Strengths, Limitations, and Recommended use. If the participant reads it unprompted, follow up: "Is this useful alongside the generated explanation, or redundant?")*
...
5. **Open Threshold sensitivity.** Before the participant looks at the tab, click **▶ Test mode** in the top-right corner of the confidence table card — this starts a 10-second timed reveal. Let them read the table normally during those 10 seconds, then the table blurs automatically and a "Time's up — What did you think these numbers meant?" prompt appears. Ask them to answer before clicking ↺ Reset to restore the table. Then point them at Linear Regression's row (lowest confidence scores in the data) and ask: "Does this change how much you trust Linear Regression's label from the Metrics tab? Would you have wanted to know this *before* reading the label, not after?" *(tests whether the robustness information is actually load-bearing for trust, and — via the timed-reveal phase — whether participants' first unprompted interpretation of the table matches its intended meaning. Note: the table now has an amber ⚠ comparability warning above it; observe whether participants read it or skip it)*
...
**Updates added 2026-06-30 (after pilot session with an engineering-practitioner participant):**
- A real user misread the Threshold Sensitivity confidence table during a pilot session, interpreting scores as a cross-model ranking of trustworthiness. Two fixes were made: (a) an amber ⚠ comparability warning banner now appears above the table; (b) each model row has a colour-coded left border in its palette colour to structurally discourage cross-row reading. The Task 5 moderator note above has been updated accordingly.
- A **timed-reveal test mode** (▶ Test mode) was added to the confidence table for use in moderated evaluation sessions. See the updated Task 5 note for the exact moderation flow.
- The Metrics tab now shows skill scores vs. Seasonal Naive baseline sub-lined under MAE and RMSE values. Task 1's note above covers the moderator observation to make.
- The Linguistic Explanations tab now includes a hand-authored Model profile block (Strengths / Limitations / Recommended use) under each model's CwW explanation. Task 2's note above covers the follow-up question.
