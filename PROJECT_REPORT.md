...
A React + Vite app (`npm run dev`, then open `http://localhost:5173`) with **no router, no charting library, and no CSS framework** — every chart is a hand-built SVG or a bar made of `<div>`s, and navigation between views is just component state, not URLs. Above the tabs, a **top summary banner** always shows the current best-ranked model (via the same `rankScore()` helper the table uses), its skill score vs. baseline, and a one-line explanation — visible regardless of which tab is open. It has five tabs, each reading a specific file:

| Tab | Reads | Shows |
|---|---|---|
| Metrics & quality | `metrics_all_models.json`, `session04_fuzzy_labels.csv`, plus a live call to a local LLM | a free-text box that classifies "what matters most for your decision" into one of four fixed preferences and deterministically re-sorts the table (confirm-before-apply, manual dropdown always available as a direct alternative — see below); a model pre-selection filter (baseline + best model by default, expandable to all 7); sortable table + bar chart; MAE and RMSE cells include a skill score vs. Seasonal Naive baseline (e.g. "+19% vs. SNaive", green/red, with a hover tooltip defining the baseline), computed as `(1 − model_error / baseline_error) × 100` — not routed through the CwW pipeline; click a row to see its forecast-vs-actual mini chart |
| Linguistic explanations | `session06_narratives.csv` (the `Structured_Explanation` column), `forecasts_all_models.json` | notebook 05's sentence per model, a forecast-vs-actual chart per model (Y-axis intentionally shared across all 7 models for comparability, not per-model min/max), followed by a hand-authored **Model profile** block (Strengths / Limitations / Recommended use, each model's Limitations including one structural-assumption sentence) for each model class, and a disclaimer that profiles describe general model-class behaviour, not a guarantee on this specific data |
| Narratives | `session06_narratives.csv` (the `Narrative` column) | notebook 06's fuller paragraph per model |
| Threshold sensitivity | `session08_sensitivity.json` | confidence heatmap + stability chart from notebook 08; an amber ⚠ comparability caption above the table states scores are only meaningful within a model's own row (with a concrete example), plus a legend note that colour indicates within-model stability, not cross-model rank; each confidence row has a per-model colour-coded left border; a moderator **Test mode** button triggers a 10-second timed reveal (countdown → blur + prompt → reset) for use during moderated evaluation sessions |
| LLM Narrative | `session09_llm_narratives.json` | a three-stage flow: blind comparison (Version A/B, identities hidden) → Reveal (real identities — "Rule-based" / "AI-rephrased" — full text, no diff yet) → opt-in **Show differences** (word-level diff highlighting what changed, plus a caveat that fluent phrasing isn't itself evidence of accuracy), with a faithfulness-flag badge if the LLM introduced an unverified number |

**The Metrics tab's free-text classifier is the only feature that calls an LLM live from the browser** — everything else reads only pre-generated static files. It reuses the exact same local integration as notebook 09 (Ollama, `llama3`, no API key, `temperature=0`, JSON-constrained output): the LLM's only output is one of five fixed labels (four preferences + "unclear"), validated against that fixed set client-side before use; a separate, plain object lookup (`PREFERENCE_TO_METRIC`) does the actual re-sorting by calling the same `setSortBy`/`setActiveMetricInfo` the existing sort-pill buttons use. The LLM never picks a metric, computes a value, or changes which models are shown.

The main file is `src/components/ForecastDashboard.jsx`, with `src/components/ForecastPlot.jsx` (per-model chart) and `src/components/MetricsPreferenceControl.jsx` (the free-text classifier) split out as separate components. The top-level shell that fetches all the files and switches between this dashboard and a "Notebooks" browsing view is `src/App.jsx`; `src/components/NotebookWindow.jsx` and `StepWindow.jsx` render that notebook-browsing view, with the descriptive text for each notebook hand-written in `src/content/steps.js` (currently covers notebooks 01–07 only — see §6).
...
### Fixed 2026-07-21 (repo-structure audit)

- **Removed `src/styles/main.css`** — the dead, unused alternate stylesheet flagged above (dark mode, unrelated class names, left over from an earlier UI version) was still being imported in `main.jsx` and shipped in every build. Confirmed via a full audit that none of its class selectors are referenced anywhere in the current JSX, then removed the file and its import. CSS bundle size dropped from 11.80 kB to 5.17 kB, confirming it really was dead weight, not just visually overridden by `app.css`'s later cascade order. Verified the Dashboard and Notebooks views both render identically after removal.
- **Fixed the broken "Preview HTML export" link** — `StepWindow.jsx` linked to `/public/<file>.html`; Vite serves the `public/` folder's contents from the site root with no `/public` prefix, so this always 404'd. Changed to `/<file>.html`; verified it now returns 200.
- **Fixed the sparkline "last N days" bug** — `selForecasts` used `.slice(0, 30)` (first 30 matching rows) while its caption claims "last N days." Harmless today only because each model has exactly 30 rows total, so first-30 and last-30 coincide; changed to `.slice(-30)` so the code is correct regardless of future data size.
- **Removed the dead `condition ? x : x` ternary** in the bar-chart label (`ModelBarChart`) — both branches returned the identical string; replaced with the plain string.
...
### Added 2026-07-21 (ahead of Interviews 4–5, driven by real evaluation-session feedback from P1–P4)

11. **Forecast-vs-actual chart (Linguistic Explanations tab):** `src/components/ForecastPlot.jsx`, a hand-built SVG line chart (actual vs. predicted, 30-day window) per model. The Y-axis is deliberately scaled from all 7 models' data, not just the model being plotted, so every chart shares one axis and is visually comparable — a note documents this so it isn't "fixed" to a per-model scale later. A separate note flags that Linear Regression's line looks flat only because its real trend (~10 MW range) is invisible against the shared ~4,130 MW axis, not because its forecast is actually constant (unlike Naive, whose forecast genuinely is one constant value for the whole window).
12. **Top summary banner:** shown above the tabs regardless of which tab is active — best-ranked model (via the same `rankScore()` the table uses), its skill score vs. baseline, and the first sentence of its CwW explanation.
13. **Model pre-selection filter:** Metrics and Linguistic Explanations tabs default to showing only the baseline (Seasonal Naive) + current best model, with a one-click "Show all 7 models" toggle; state is shared between the two tabs. Threshold Sensitivity and LLM Narrative are unaffected.
14. **Model-class assumption disclosures:** each model's Limitations field in `MODEL_PROFILES` now includes one sentence naming its core structural assumption (e.g. Naive: "assumes tomorrow equals today"; SARIMA: "assumes a stationary linear process"), plus a standing disclaimer that these profiles describe general model-class behaviour, not a performance guarantee on this specific data.
15. **LLM Narrative tab restructured into a three-stage flow:** blind comparison (Version A/B, identities hidden) → Reveal (real identities — "Rule-based" / "AI-rephrased" descriptor labels — full text, no diff) → an explicit opt-in **Show differences** button before the word-level diff view appears. This was a deliberate change from an earlier version that showed the diff immediately on reveal — the intent is that a participant reads the plain texts under their real labels first, before being shown *what changed*, so the diff-highlighting doesn't front-run their own read of the text. The diff itself is a hand-rolled LCS word-level comparison (`diffWords`/`NarrativeDiff`, no external diff library) with a fluency caveat line ("fluent phrasing is not itself evidence of statistical accuracy").
16. **Threshold Sensitivity comparability caption reworded** with a concrete example (0.85 vs. 0.85 not being the same thing across models) and a new legend note that colour indicates within-model stability, not cross-model rank.
17. **Free-text preference classifier (Metrics tab):** `src/components/MetricsPreferenceControl.jsx` — a text box that classifies a plain-language description of what matters most ("I need to avoid big cost spikes") into one of four fixed preferences via a live call to the same local Ollama/`llama3` integration notebook 09 uses (no new provider, no API key, `temperature=0`, JSON-constrained output). The LLM's only output is one of five fixed labels (four preferences + "unclear"); everything downstream — which metric to sort by — is a plain, deterministic object lookup, not an LLM decision. Confirm-before-apply, plus an always-available manual dropdown. The first prompt version guessed a category on genuinely ambiguous input (e.g. "not sure, just pick something reasonable") rather than saying "unclear" — fixed by adding explicit negative examples for the fallback case to the system prompt, verified by re-testing.
18. **SNaive tooltip rebuilt as a CSS-hover tooltip** (`.hint`/`.hint__bubble` in `app.css`) instead of the native `title` attribute, which wasn't reliably rendering its tooltip bubble across browsers.
...
3. Keep `ollama serve` running (with `llama3` pulled) whenever demoing the dashboard — the Metrics tab's free-text preference classifier calls it live; every other tab works fine with Ollama stopped.
